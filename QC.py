...
def patient_name(root):
    ...
    # determine file type
    files = []
    with open(root, 'r') as read_obj:
        csv_reader = reader(read_obj)
        for row in csv_reader:
            if os.path.isfile(''.join(row)) and (''.join(row).endswith('.dcm') or ''.join(row).endswith('.mha') or ''.join(row).endswith('.nii') or ''.join(row).endswith('.gz') or ''.join(row).endswith('.mat')):
                files.append(''.join(row))
            else:
                print('The image ', row, ' does not exist or is not an acceptable format and will be omitted from the results.')
    ...
    if folders_flag == "False":
        ...
        dicom_subjects = [''.join(d) for d in csv_reader if os.path.isfile(d)]
        subjects_number = []
        for i in range(len(dicom_subjects)):
            subjects_number.append(
                len([row 
            for row in dicom_subjects[i] 
            if row.endswith('.dcm')]))
...
def volume_dicom(scans, name):
    ...
    if folders_flag == "False":
        ...
        name_vale = name
    # specify image information and measurement types        
    tags = {
             'ID': name_vale,
...
# method used to convert a tsv file to a dataframe
...
# method used to print a message box
...
# save user input parameters to args
if __name__ == '__main__':
    ...
    parser.add_argument('inputcsv',
                        help = "input csv containing paths to images.",
                        type=str)
    parser.add_argument('outputdir', 
                        help = "output location of UserInterface directory, which contains the results tsv, IQM csv, and MRQy images.")
    ...
    if path_exists is False:
        print('The output directory does not exist. The current directory will be used as an output directory.')
        args.outputdir = os.getcwd()

    if ''.join(args.inputcsv).endswith('.csv') is False:
        print ('The input is not a csv file. The program will now terminate.')
        sys.exit(0)

    root = args.inputcsv
   
    # save image information as variables
    if args.r == 0:
        folders_flag = "False"
    else: 
        folders_flag = args.r
    ...
    # print(os.getcwd())
    print_forlder_note = args.outputdir + os.sep + 'UserInterface' 
    ...
    # change flags based on file format
